...
class SSWSFNCT_Nag
{
    ...
    {
        if (isset($_GET[SSWSFunctions::OPTION_ADMIN_NOTICE_KEY]) && current_user_can('install_plugins')) {
            // Add user meta
            global $current_user;
            add_user_meta($current_user->ID, SSWSFunctions::OPTION_ADMIN_NOTICE_KEY, '1', true);

            // Build redirect URL
            $query_params = $this->get_admin_querystring_array();
            unset($query_params[SSWSFunctions::OPTION_ADMIN_NOTICE_KEY]);
    ...
    private function bind()
    {
        ...
        $hide_notice = get_user_meta($current_user->ID, SSWSFunctions::OPTION_ADMIN_NOTICE_KEY, true);
    ...
    {
        $date_string = get_site_option(SSWSFunctions::OPTION_INSTALL_DATE, '');
        if ($date_string == '') {
    ...
    public static function insert_install_date()
    {
        ...
        add_site_option(SSWSFunctions::OPTION_INSTALL_DATE, $date_string, '', 'no');
    ...
    public function display_admin_notice()
    {
        ...
        $query_params = $this->get_admin_querystring_array();
        $query_string = '?' . http_build_query(array_merge($query_params, array(SSWSFunctions::OPTION_ADMIN_NOTICE_KEY => '1')));

        echo '<div class="updated"><p>';
        printf(__("You are running <b>SSWS Functions</b> - custom WordPress development"), 'https://www.seatoskywebsolutions.ca/', $query_string);
        echo "</p></div>";
