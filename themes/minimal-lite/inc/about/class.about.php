 ...
 * @since 1.0.0
 */
if ( ! class_exists( 'Minimal_Lite_About' ) ) {
	/**
	 * Singleton class used for generating the about page of the theme.
	 */
	class Minimal_Lite_About {
		/**
		 ...
		 *
		 * @var Minimal_Lite_About $instance The  TI_About_Page instance.
		 */
		...
		public static function init( $config ) {
			if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Minimal_Lite_About ) ) {
				self::$instance = new Minimal_Lite_About;
		...
		public function ti_about_page_render() {
			...
			if ( ! empty( $welcome_title ) || ! empty( $welcome_content ) || ! empty( $this->tabs ) ) {
				...
				echo '<div class="wrap about-wrap thememattic-about-wrap">';
				...
				echo '<a href="https://thememattic.com/" target="_blank" class="wp-badge epsilon-welcome-logo"></a>';
    ...
    return add_query_arg(
		...
		public function getting_started() {
			...
			?>
			<div class="feature-section thememattic-section thememattic-section-getting-started three-col">
				<?php foreach ( $content as $item ) : ?>
					<?php $this->render_grid_item( $item ); ?>
				<?php endforeach; ?>
			</div><!-- .feature-section .thememattic-section -->
			<?php
		}
		...
		public function support() {
			...
			?>
			<div class="feature-section thememattic-section thememattic-section-support three-col">
				<?php foreach ( $content as $item ) : ?>
					<?php $this->render_grid_item( $item ); ?>
				<?php endforeach; ?>
			</div><!-- .feature-section .thememattic-section -->
		...
		public function free_pro() {
			...
			if ( ! empty( $free_pro ) ) {
				if ( ! empty( $free_pro['free_theme_name'] ) && ! empty( $free_pro['pro_theme_name'] ) && ! empty( $free_pro['features'] ) && is_array( $free_pro['features'] ) ) {
					...
					echo '<tr><td style="border-top:0;"></td><td style="border-top:0;"></td><td style="border-top:0;" class="only-pro"><a href="https://thememattic.com/minimal-lite-pro-demos/" target="_blank" >';
					echo esc_html ('Minimal Lite Pro Live Preview', 'minimal-lite');
					echo '</a></td></tr>';
		...
		public function style_and_scripts( $hook_suffix ) {
			...
			wp_enqueue_style( 'about-page-css', get_template_directory_uri() . '/inc/about/css/about.min.css' );

			if ( 'appearance_page_' . $this->theme_slug . '-welcome' == $hook_suffix ) {

				wp_enqueue_script( 'about-page-js', get_template_directory_uri() . '/inc/about/css/about.min.js', array( 'jquery' ) );
        ...
        'activating_string'        => __( 'Activating', 'minimal-lite' )
        )
    );
